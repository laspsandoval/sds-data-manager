...
1. An HTTPS API to upload files to an S3 bucket (_in development_)
2. An S3 bucket to contain uploaded files
3. An HTTPS API to query and download files from the S3 bucket (_in development_)
...
To install all extras, or partial extras
...
This will install the dependencies from `poetry.lock`, ensuring that consistent
